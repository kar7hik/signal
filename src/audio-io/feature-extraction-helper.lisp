...
(defun apply-filter-to-audio (splitted-frames &key (filter-function 'hamming))
  (iter
    ...
    (collect (apply-filter-to-frame (elt splitted-frames i)
...
(defun apply-fft-to-audio (filtered-audio-data  &key (nfft *nfft*))
  ...
  (let ((total-num-frames (length filtered-audio-data)))
    (iter
      (for i :from 0 :below total-num-frames)
      (collect (apply-fft-to-frame (elt filtered-audio-data i) :nfft nfft)))))
...
(defun find-power-spectrum-audio (frames NFFT)
  ...
  (iter
    ...
    (collect (apply-power-spectrum frame NFFT))))
